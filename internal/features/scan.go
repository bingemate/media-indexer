...
func (s *TVScanner) processTVEpisodes(tvList *pkg.AtomicTVEpisodeList, destination string) error {
	...
	for mediaFile, media := range tvList.GetAll() {
		...
		log.Printf("Processed %-60s - %s - %s s%02de%02d", mediaFile.Filename, media.TvShowName, media.Year(), mediaFile.Season, mediaFile.Episode)
		pkg.AppendJobLog(fmt.Sprintf("Processed %-60s - %s - %s s%02de%02d", mediaFile.Filename, media.TvShowName, media.Year(), mediaFile.Season, mediaFile.Episode))
